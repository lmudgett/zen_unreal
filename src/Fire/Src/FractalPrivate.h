...
#include "Engine.h"
// x64 port: removed the 1998 "!!!TEMPORARY!!" block that force-defined
// ASM=1 and un-poisoned __asm; the build-wide ASM setting now applies,
// so the C fallback paths are used on x64.
...
// x64 port: the fractal texture classes are mirrored by *.uc declarations —
// script layout rules match pack(4), the 1998 /Zp4 default.
#pragma pack (push,4)
...
#pragma pack (pop) // x64 port: end of script-mirrored classes
