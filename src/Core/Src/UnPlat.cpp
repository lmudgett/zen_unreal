...
// cross-platform port: this file is the shared Core platform layer. The
// Windows-specific primitives (DLL loading, clipboard, message boxes, GUID,
// shell-launch, file move/copy, module path, memory query) are forked with
// #if _WIN32 / #else POSIX branches; everything else (config parsing,
// logging, slow-task UI, init flow) is portable and shared.
#if _WIN32
	...
	#include <io.h>
	#include <float.h>
	#include <intrin.h>		// x64 port: __cpuid (inline asm is unavailable on x64)
#else
	#include <dlfcn.h>		// dlopen/dlsym/dlclose
	#include <unistd.h>		// readlink, sysconf
	#include <glob.h>		// appFindFiles
	#include <cstdlib>		// system(), realpath
	#include <cstdint>
	#if defined(__APPLE__)
		#include <sys/sysctl.h>	// sysctl(HW_MEMSIZE) — _SC_PHYS_PAGES is Linux-only
	#endif
#endif
#include <stdio.h>
#include <time.h>
#include <sys/stat.h>
	...
	#include <utime.h>
#endif
#include <chrono>		// cross-platform port: portable high-resolution timing

#include "Core.h"

#if !_WIN32
// cross-platform port: MSVC CRT helpers the codebase uses, on POSIX. The
// _stat/_utime macros map both the struct tag and the function name.
#define _stat  stat
#define _utime utime
static char* _strdate( char* B ) { time_t t=time(0); strftime(B,9,"%m/%d/%y",localtime(&t)); return B; }
static char* _strtime( char* B ) { time_t t=time(0); strftime(B,9,"%H:%M:%S",localtime(&t)); return B; }
#endif
...
#else
		// cross-platform port: physical memory from sysconf.
		long Pages = sysconf(_SC_AVPHYS_PAGES), PageSize = sysconf(_SC_PAGE_SIZE);
		Out->Logf( "Memory available: Phys=%iK", (INT)((QWORD)Pages*PageSize/1024) );
#endif
		return 1;
	}
...
#else
		// cross-platform port: re-exec self via the shell.
		char Line[512];
		appSprintf( Line, "\"%s/%s\" %s &", appBaseDir(), appPackage(), Cmd );
		if( system(Line) == -1 ) Out->Log( "Relaunch failed" );
#endif
		appRequestExit();
		return 1;
	}
...
#else
	// cross-platform port: system clipboard is provided by the windowing
	// layer (SDL_SetClipboardText) once SDLDrv lands; no-op in headless Core.
	(void)Str;
#endif
	unguard;
}
...
#else
	Result = ""; // cross-platform port: see ClipboardCopy
#endif
	unguard;
}
...
#elif _WIN32
	// x64 port: the 1998 inline-asm CPUID can't compile on x64 (ASM==0), and
	// the old all-zero fallback left GIsMMX/GIsPentiumPro false — which forced
	// the first-run config to 320x240 + 11kHz audio and the slow C render
	// paths, and printed "Unknown processor". The __cpuid intrinsic gives the
	// real vendor string and feature bits (every x64 CPU has MMX+CMOV+SSE2).
	int Regs[4] = { 0, 0, 0, 0 };
	__cpuid( Regs, (int)i );
	*A = (DWORD)Regs[0];
	*B = (DWORD)Regs[1];
...
void appInit()
{
	...
	char Comp[256]="";
#if _WIN32
	DWORD Size=ARRAY_COUNT(Comp);
	GetComputerName( Comp, &Size );
#else
	gethostname( Comp, ARRAY_COUNT(Comp) ); // cross-platform port
#endif
	...
	*d++ = 0;
	...
	// x64 port: the VC5 small-block heap tuning API (__p__amblksiz) no longer
	// exists; the modern CRT heap needs no granularity tweak.
	...
	// Working set. (x64 port: SIZE_T, not DWORD.)
	SIZE_T WsMin=0, WsMax=0;
	GetProcessWorkingSetSize( GetCurrentProcess(), &WsMin, &WsMax );
	debugf( NAME_Init, "Working set: %X / %X", (DWORD)WsMin, (DWORD)WsMax );
...
#else
	// cross-platform port: portable machine info (no Win32/CPUID). appCycles
	// is now a nanosecond tick, so GSecondsPerCycle = 1e-9 exactly (matches
	// the Windows empirical calibration, which also converges here).
	GSecondsPerCycle = 1e-9;
#if defined(__APPLE__)
	// macOS: _SC_PHYS_PAGES is Linux-only (won't even compile) — use sysctl.
	{
		QWORD Mem = 0; size_t Len = sizeof(Mem);
		INT Mib[2] = { CTL_HW, HW_MEMSIZE };
		if( sysctl( Mib, 2, &Mem, &Len, NULL, 0 )==0 )
			GPhysicalMemory = (INT)( Mem > 0x7FFFFFFF ? 0x7FFFFFFF : Mem );
	}
#else
	GPhysicalMemory  = (INT)((QWORD)sysconf(_SC_PHYS_PAGES)*sysconf(_SC_PAGE_SIZE));
#endif
	GPageSize        = (INT)sysconf(_SC_PAGE_SIZE);
	GProcessorCount  = (INT)sysconf(_SC_NPROCESSORS_ONLN);
	GIsMMX = GIsPentiumPro = 1; // legacy flags (asm paths disabled via ASM=0)
	debugf( NAME_Init, "Memory total: Phys=%iK", GPhysicalMemory/1024 );
	debugf( NAME_Init, "CPU Page size=%i, Processors=%i", GPageSize, GProcessorCount );
#endif
...
#else
	// cross-platform port: x87 precision control is x86-only and irrelevant
	// under SSE math; no-op elsewhere.
	(void)Enable;
#endif
	unguard;
}
...
CORE_API void* appGetDllHandle( const char* Filename )
{
	...
	return Result;
#else
	// cross-platform port: dlopen. Try the name as given, then with the
	// platform shared-object prefix/suffix (lib<name>.so / .dylib).
	void* Result = dlopen( Filename, RTLD_LAZY );
	if( !Result )
	{
		char Temp[300];
#if __APPLE__
		appSprintf( Temp, "lib%s.dylib", Filename );
#else
		appSprintf( Temp, "lib%s.so", Filename );
#endif
		Result = dlopen( Temp, RTLD_LAZY );
	}
	return Result;
#endif
	unguard;
...
#else
	dlclose( DllHandle ); // cross-platform port
#endif
	unguard;
}
...
#else
	return dlsym( DllHandle, ProcName ); // cross-platform port
#endif
	unguard;
}
...
void appDebugBreak()
{
	...
	__builtin_trap(); // cross-platform port
#endif
	unguard;
}
...
// Get time in seconds.
//
// cross-platform port: std::chrono::steady_clock replaces the Win32
// QueryPerformanceCounter / __rdtsc timing. It is monotonic and
// high-resolution on every platform (and avoids x86-only rdtsc, so this
// path also works on ARM/Apple Silicon). appCycles now returns a
// nanosecond tick; the empirical GSecondsPerCycle calibration in appInit
// converges to 1e-9 automatically, so clock()/unclock() stat math is
// unchanged. (Was: QPC seconds + rdtsc cycles — Windows-only.)
...
{
	static const std::chrono::steady_clock::time_point Start = std::chrono::steady_clock::now();
	return (DWORD)std::chrono::duration_cast<std::chrono::nanoseconds>(
...
#else
	// cross-platform port: localtime (millisecond precision not provided).
	time_t t = time(0);
	struct tm* lt = localtime( &t );
	Year=lt->tm_year+1900; Month=lt->tm_mon+1; DayOfWeek=lt->tm_wday; Day=lt->tm_mday;
	Hour=lt->tm_hour; Min=lt->tm_min; Sec=lt->tm_sec; MSec=0;
#endif
	unguard;
}
...
#if _WIN32
	INT Code = (INT)(PTRINT)ShellExecute(NULL,"open",URL,Parms,"",SW_SHOWNORMAL);
	if( Code<=32 )
		appSprintf( Error256, LocalizeError("UrlFailed") );
#else
	// cross-platform port: xdg-open (Linux) / open (macOS).
	char Line[1024];
#if __APPLE__
	appSprintf( Line, "open '%s%s%s' &", URL, Parms&&*Parms?"?":"", Parms?Parms:"" );
#else
	appSprintf( Line, "xdg-open '%s%s%s' &", URL, Parms&&*Parms?"?":"", Parms?Parms:"" );
#endif
	if( system(Line) == -1 && Error256 )
		appSprintf( Error256, LocalizeError("UrlFailed") );
#endif
	unguard;
}
...
#else
	UBOOL Success = ( rename( Src, Dest )==0 ); // cross-platform port
	if( !Success && appCopyFile(Src,Dest) )     // cross-device: copy then unlink
		{ unlink(Src); Success = 1; }
#endif
	if( !Success )
...
#else
	// cross-platform port: portable byte copy.
	UBOOL Success = 0;
	FILE* S = fopen( Src, "rb" );
	if( S )
	{
		...
		if( D )
		{
			...
			while( (n=fread(Buf,1,sizeof(Buf),S))>0 )
				if( fwrite(Buf,1,n,D)!=n ) { Success=0; break; }
			fclose( D );
		}
		fclose( S );
	}
...
#else
	// cross-platform port: the windowing layer shows a real dialog once
	// SDLDrv lands; headless Core logs to stderr.
...
#else
	fprintf( stderr, "QUESTION (assuming yes): %s\n", TempStr ); // cross-platform port
	return 1;
#endif
	unguard;
}
...
void appError( const char* Msg )
{
	...
	appDebugBreak(); // cross-platform port: was ::DebugBreak()
#else
	if( GIsCriticalError )
	{
...
void FGlobalPlatform::BeginSlowTask( const char* Task, UBOOL StatusWindow, UBOOL Cancelable )
{
	...
	{
		SendMessage( (HWND)(UINT_PTR)hWndProgressBar, PBM_SETRANGE, (WPARAM)0, MAKELPARAM(0, 100) );
		SendMessage( (HWND)(UINT_PTR)hWndProgressText, WM_SETTEXT, (WPARAM)0, (LPARAM)Task );
		UpdateWindow( (HWND)(UINT_PTR)hWndProgressText );
	}
...
UBOOL VARARGS FGlobalPlatform::StatusUpdatef( INT Numerator, INT Denominator, const char* Fmt, ... )
{
	...
	{
		SendMessage( (HWND)(UINT_PTR)hWndProgressText, WM_SETTEXT, (WPARAM)0, (LPARAM)TempStr );
		SendMessage( (HWND)(UINT_PTR)hWndProgressBar, PBM_SETPOS, (WPARAM)(Denominator ? 100*Numerator/Denominator : 0), (LPARAM)0 );
	}
...
#else
	// cross-platform port: RFC-4122-ish random GUID (rand is seeded in
	// appInit). Package GUIDs only need uniqueness, not crypto strength.
	DWORD* W = (DWORD*)&Result;
	for( INT i=0; i<4; i++ )
		W[i] = ((DWORD)rand()<<20) ^ ((DWORD)rand()<<8) ^ (DWORD)rand();
	W[1] = (W[1] & 0xffff0fff) | 0x00004000; // version 4
	W[2] = (W[2] & 0x3fffffff) | 0x80000000; // variant
#endif
	return Result;
...
#if !_WIN32
// cross-platform port: the launcher records argc/argv here (Core has no
// GetCommandLine). appExecPath resolves the running executable's path.
static char GPosixCmdLine[4096]="";
static char GPosixExecPath[1024]="";
CORE_API void appSetCmdLine( int argc, char** argv )
{
	if( argc>0 ) appStrcpy( GPosixExecPath, argv[0] );
	GPosixCmdLine[0]=0;
	for( int i=1; i<argc; i++ )
	{
		if( i>1 ) appStrcat( GPosixCmdLine, " " );
		appStrcat( GPosixCmdLine, argv[i] );
	}
}
static const char* appExecPath()
{
	if( !GPosixExecPath[0] )
	{
#if __APPLE__
		appStrcpy( GPosixExecPath, "./Unreal" ); // _NSGetExecutablePath in the SDL launcher
#else
		ssize_t n = readlink( "/proc/self/exe", GPosixExecPath, ARRAY_COUNT(GPosixExecPath)-1 );
		if( n>0 ) GPosixExecPath[n]=0; else appStrcpy( GPosixExecPath, "./Unreal" );
#endif
	}
	return GPosixExecPath;
}
#endif
...
CORE_API const char* appCmdLine()
{
	...
	return CmdLine;
#else
	return GPosixCmdLine; // cross-platform port: set via appSetCmdLine
#endif
	unguard;
}
...
#else
		appStrncpy( BaseDir, appExecPath(), ARRAY_COUNT(BaseDir) ); // cross-platform port
#endif
...
#else
		appStrncpy( Tmp, appExecPath(), ARRAY_COUNT(Tmp) ); // cross-platform port
#endif
		while( appStrchr(End,'\\') )
			End = appStrchr(End,'\\')+1;
		while( appStrchr(End,'/') )
