...
#include <string.h>
	...
	#include <io.h>
#else
	#include <glob.h>		// cross-platform port: appFindFiles
...
void FArray::Realloc( INT ElementSize )
{
	...
	// x64 port / security: ArrayMax is frequently file-controlled (array
	// element counts are read straight from untrusted packages). The byte
	// count is computed in 64-bit and bounded so a crafted count cannot
	// overflow the 32-bit allocation size and under-allocate — which would
	// turn the subsequent element writes into a heap buffer overflow. Reject
	// negative or >2GB requests outright (no legitimate array approaches this).
	if( ArrayMax<0 || (QWORD)(DWORD)ArrayMax*(QWORD)(DWORD)ElementSize > (QWORD)0x7fffffff )
		appErrorf( "FArray::Realloc: bad array size %i * %i", ArrayMax, ElementSize );
...
CORE_API void* appRealloc( void* Ptr, INT NewSize, const char* Tag )
{
	...
	unguardf(( "%08X %i %s", (DWORD)(PTRUINT)Ptr, NewSize, Tag ));
}
...
// x64 port: the original had no C fallback for appRound (only the x86
// fistp inline in UnVcWin32.h). Note (int+.5) rounds to (int+1), matching
...
#if _WIN32
	_finddata_t Found;
	// x64 port: _findfirst returns a pointer-sized handle; 'long' truncated it.
	intptr_t hFind = _findfirst( Spec, &Found );
	if( hFind != -1 )
	{
...
#else
	// cross-platform port: glob() matches the wildcard spec; return bare
	// filenames (basename) to match the Win32 _findfirst behavior.
	glob_t g;
	if( glob( Spec, 0, NULL, &g )==0 )
	{
		for( size_t i=0; i<g.gl_pathc; i++ )
		{
			char Tmp[1024];
			appStrncpy( Tmp, g.gl_pathv[i], ARRAY_COUNT(Tmp) );
			new(Result)FString( basename(Tmp) );
		}
	}
	globfree( &g );
#endif
...
CORE_API INT appFprintf( FILE* F, const char* Fmt, ... )
{
	char Temp[32768];
	INT Result;
	GET_VARARGS_RESULT(Temp,Fmt,Result);
...
{
	INT Result;
	GET_VARARGS_RESULT(Dest,Fmt,Result);
	return Result;
}

// x64 port: appGetVarArgs() was removed. It relied on reading the caller's
// varargs off the stack, which is impossible under the x64 calling
// convention; GET_VARARGS now expands va_start in the caller instead.
...
{
	return (char*)strstr(String,Find);
}
...
{
	// x64 port: the hand-scheduled MMX path was removed; the modern CRT
	// memset already uses the widest SIMD available on the host CPU.
	return memset( Dest, C, Count );
}

CORE_API void* appLargeMemcpy( void* Dest, const void* Src, INT Count )
{
	// x64 port: as above, modern CRT memcpy replaces the MMX copy loop.
