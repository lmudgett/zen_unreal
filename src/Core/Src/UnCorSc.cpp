...
void UObject::execClassContext( FFrame& Stack, BYTE*& Result )
{
	...
	else
	{
		...
		Stack.Code += wSkip;
		// x64 port: bSize from 32-bit-era bytecode may undercount object-sized
		// results (translated where statically resolvable, but virtual function
		// results are not); always clear at least a full pointer slot.
		if( Result )
			appMemset( Result, 0, Max( (INT)bSize, (INT)sizeof(void*) ) );
		GBoolAddr = NULL;
	}
