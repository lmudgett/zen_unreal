			...
			for( INT j=0; j<Import.SourceLinker->ExportMap.Num(); j++ )
			{
				...
				&&	Source.ClassPackage	== Import.ClassPackage )
				{
					...
					if( !(Source.ObjectFlags & RF_Public) )
						appThrowf( LocalizeError("FailedImportPrivate"), *Source.ClassName, Import.SourceLinker->LinkerRoot->GetName(), *Source.ObjectName );
...
private:
	...
	UObject* CreateExport( INT Index )
	{
		...
		unguardf(( "(%s %i %08X)", *ExportMap(Index).ObjectName, Tell(), (DWORD)(PTRUINT)File ));
	}
	...
	void LoadObject( UObject* Object )
	{
		...
		unguardf(( "(%s %i==%i/%i (%i %i) %08X)", Object->GetFullName(), Tell(), Pos, Eof, ExportMap( Object->GetLinkerIndex() ).SerialOffset, ExportMap( Object->GetLinkerIndex() ).SerialSize, (DWORD)(PTRUINT)File ));
	}
	...
	FArchive& operator<<( UObject*& Object )
	{
		...
		*this << AR_INDEX(Index);
		...
		return *this;
		unguardf(( "(%s %i %08X))", GetFullName(), Tell(), (DWORD)(PTRUINT)File ));
	}
