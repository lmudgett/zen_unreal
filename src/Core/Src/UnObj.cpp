...
#include <stdlib.h>	// atexit (shutdown-lifecycle guard, see FObjectManager::Init)
...
void FObjectManager::PreRegister( UObject* InObject, FName InPackageName )
{
	...
	else
	{
		...
		// x64 port: the chain pointer was stored in the 32-bit LinkerIndex,
		// which truncates 64-bit addresses. HashNext is a real pointer and
		// is unused until Register() hashes the object, so chain there.
		InObject->HashNext = AutoRegister;
...
void FObjectManager::Init()
{
	...
	// x64 port: on a forced exit (Ctrl+C / console-close / kill) the C runtime
	// runs the static autoclass* UClass destructors WITHOUT the clean
	// GObj.Exit(), so Initialized stays 1 and ~UObject's check(IsValid()) fires
	// once the Objects table is torn down ("Invalid object index" crash). This
	// atexit flips Initialized off first: the standard guarantees an atexit
	// callback runs before the destructors of any static object constructed
	// before its registration (i.e. all the autoclass* globals), so every
	// out-of-lifecycle ~UObject then takes the safe early-out. (The captureless
	// lambda is defined in this member function, so it may touch the private
	// static Initialized and still convert to atexit's void(*)().)
	atexit( []{ FObjectManager::Initialized = 0; } );
	...
	// x64 port: the chain lives in HashNext now; see PreRegister.
	UObject* Next;
	for( UObject* Object=AutoRegister; Object!=NULL; Object=Next )
	{
		Next = Object->HashNext;
		Object->HashNext = NULL;
...
)
{
	...
	INT ClassSize = 0;
	INT CppLayout = 0; // x64 port: see ClassSize preservation below
	...
	else
	{
		...
		if( Obj->IsA( UClass::StaticClass ) )
		{
			...
			// x64 port: for registered intrinsic classes PropertiesSize is the
			// compiled C++ size, which LinkOffsets pins the script layout to;
			// preserve it across the in-place replacement like Constructor,
			// along with the CppLayoutSize pin source.
			ClassSize   = ((UClass*)Obj)->PropertiesSize;
			CppLayout   = ((UClass*)Obj)->CppLayoutSize;
		}
		Index = Obj->Index;
		Obj->~UObject();
	...
	if( Obj->IsA( UClass::StaticClass ) )
	{
		...
		// x64 port: see above — keep the compiled C++ layout size.
		if( ClassSize )
			((UClass*)Obj)->PropertiesSize = ClassSize;
		if( CppLayout )
			((UClass*)Obj)->CppLayoutSize = CppLayout;
	}
...
)
{
	...
	// x64 port: replacing a UClass in place (a package loading over a registered
	// intrinsic class) re-runs the C++ default constructor below, whose member
	// initializer resets UStruct::CppLayoutSize to 0 — wiping the pin that
	// AllocateObject just preserved. UStruct::LinkOffsets then falls back to the
	// entry PropertiesSize; at load time that value is still the registered C++
	// size (so the game worked), but the script compiler overwrites it with the
	// SUPERCLASS size mid-parse, so every reparsed intrinsic class was pinned to
	// its super's size and default-property import wrote past the undersized
	// Defaults allocation (the heap corruption that killed -make -remake=Engine).
	// Preserve the pin across the constructor call.
	INT SavedCppLayout = Result->IsA(UClass::StaticClass) ? ((UClass*)Result)->CppLayoutSize : 0;
	(*InClass->Constructor)( Result );
	if( SavedCppLayout )
		((UClass*)Result)->CppLayoutSize = SavedCppLayout;
	unguard;
...
)
{
	...
	if( File == NULL )
	{
		// x64 port: code-only make (-remake) - the source art behind retail
		// content #exec imports is not in the repo. If the target object
		// already exists (pre-loaded from the retail package), resolve to it
		// silently; only a genuinely missing object warrants the warning.
		UObject* Existing = Name!=NAME_None ? FindObject( Class, InParent, *Name ) : NULL;
		if( Existing )
			return Existing;
