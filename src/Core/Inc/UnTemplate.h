...
{
	// x64 port: align through a pointer-sized integer; DWORD truncated.
	return (T)(((PTRUINT)Ptr + Alignment - 1) & ~((PTRUINT)Alignment-1));
}
...
#define STRUCT_OFFSET( struc, member ) \
	( (INT)(PTRINT)&((struc*)NULL)->member ) /* x64 port: via PTRINT */
...
public:
	...
	// x64 port: must be a template friend to match the operator<< template
	// defined in Core.h; the old non-template friend never matched it and
	// modern MSVC enforces the protected access the old compiler ignored.
	template< class U > friend FArchive& operator<<( FArchive& Ar, TArray<U>& A );
