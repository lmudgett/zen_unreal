...
// x64 port: for size_t/ptrdiff_t used by the base type definitions below.
#include <stddef.h>
...
// x64 port: handles are pointer-sized; DWORD would truncate them.
#ifndef _WINDOWS_
	#define HANDLE void*
...
// Variable arguments.
// x64 port: the original appGetVarArgs() walked the caller's stack to find
// the variadic args, which only worked with 32-bit cdecl. On x64 varargs
// travel in registers, so va_start must expand in the calling function
// itself. 'fmt' must be the caller's last named parameter.
#include <stdarg.h>
#include <stdio.h>
#define GET_VARARGS(msg,fmt) \
	{ va_list __VaList; va_start(__VaList,fmt); vsprintf(msg,fmt,__VaList); va_end(__VaList); }
#define GET_VARARGS_RESULT(msg,fmt,result) \
	{ va_list __VaList; va_start(__VaList,fmt); result = vsprintf(msg,fmt,__VaList); va_end(__VaList); }
...
typedef size_t              SIZE_T;     // Corresponds to C size_t; pointer-sized on x64.
typedef ptrdiff_t           PTRINT;     // x64 port: signed integer wide enough for a pointer.
typedef size_t              PTRUINT;    // x64 port: unsigned integer wide enough for a pointer.
