...
// x64 port: object references embedded in bytecode are pointer-sized in
// memory (SerializeExpr's XFER(UObject*) writes them at native width);
// the old VM read them with ReadInt, truncating them on 64-bit.
inline void* FFrame::ReadPtr()
{
