...
public:
	...
	inline BYTE* PushBytes( INT AllocSize, INT Align )
	{
		...
		// x64 port / security: this guard was `debug()` (compiled out in
		// Release). A negative AllocSize — e.g. from a 32-bit `Count*sizeof(T)`
		// that overflowed on file-controlled Count — would move Top backwards,
		// pass the End check, and hand back an unreserved pointer. Make it a
		// live check so such a request aborts instead of corrupting the pool.
		if( AllocSize<0 )
			appErrorf( "FMemStack::PushBytes: bad alloc size %i", AllocSize );
		debug((Align&(Align-1))==0);
		debug(Top<=End);

		// Try to get memory from the current chunk.
		// x64 port: align through a pointer-sized integer; INT truncated.
		BYTE* Result = (BYTE *)(((PTRUINT)Top+(Align-1))&~((PTRUINT)Align-1));
		...
		if( Top > End )
		{
			...
			Result = (BYTE *)(((PTRUINT)Top+(Align-1))&~((PTRUINT)Align-1));
			Top    = Result + AllocSize;
		}
		return Result;
		unguardSlow;
