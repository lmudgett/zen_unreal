...
public:
	...
	// x64 port: modern MSVC instantiates all TArray members for the
	// dllexported FPackageMap subclass, so FindItem/RemoveItem need this.
	UBOOL operator==( const FPackageInfo& Other ) const
	{
		return Linker==Other.Linker && Parent==Other.Parent;
	}
};
...
#define DECLARE_CLASS( TClass, TSuperClass, TStaticFlags ) \
	...
	void* operator new( size_t Size, EInternal* Mem ) \
		{ return (void*)Mem; } \
	/* x64 port: TClass without parens — value-initialization (with parens) */ \
	/* zeroes the whole object under modern C++, wiping the template/defaults */ \
	/* copy that ConstructObject applies BEFORE calling this; 1998 compilers */ \
	/* never zeroed here. */ \
...
class CORE_API UObject : public FUnknown
{
	...
	friend class UStruct;	// x64 port: LinkOffsets pins Object.uc's mirrored member offsets to the real C++ layout
