...
class CORE_API UObjectProperty : public UProperty
{
	...
	void Link( FArchive& Ar, UProperty* Prev )
	{
		// x64 port: the generated C++ headers wrap classes in #pragma pack(4),
		// so object pointers are 4-aligned in native layouts; aligning to
		// sizeof(UObject*) here would diverge from the compiled classes.
