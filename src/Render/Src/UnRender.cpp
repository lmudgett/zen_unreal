...
// x64 port: was [1200] ("max y res 1200!!") — indexed by absolute screen Y, so
// a 4K (2160) / 8K (4320) fullscreen desktop overran it and crashed the world
// rasterizer in OccludeBsp. Sized for up to 8K height; 8 bytes/entry (64KB static).
FRasterSpan HackRaster[8192];
...
)
{
	...
	// x64 port: `i++,Pt++` advanced Pt at the END of each iteration, so the loop
	// projected Pts[0] twice and never Pts[7] -- the screen box missed any extent
	// that corner alone contributed, and the too-small box made the span test cull
	// mesh actors whose only visible sliver fell in the missed strip (deterministic
	// Skaarj-behind-crates vanish in Vortex2; a latent 1998 bug, magnified at high
	// resolutions). Advance first: project Pts[1]..Pts[7].
	for( INT i=1; i<8; i++ )
	{
		Pt++;
		FLOAT RZ = Frame->Proj.Z / Pt->Point.Z;
...
{
	FCheckResult Hit;
	// x64 port: retail traced TRACE_VisBlocking (level+movers) only, so pawns
	// never occluded coronas and the flare shone straight through anyone in
	// front of the light (invisible at 640x480, glaring at 4K). Add
	// TRACE_Pawns, excluding the view actor as trace source since the ray
	// starts inside the viewer's own collision cylinder. A blocked ray just
	// stops refreshing Bright, so the existing decay fades the corona out
	// smoothly instead of popping.
	if
	(	Light->bCorona
	&&	Light->Skin
	&& !Light->bDeleteMe
	&&  Frame->Level->SingleLineCheck( Hit, Frame->Viewport->Actor, Light->Location, Frame->Coords.Origin, TRACE_VisBlocking|TRACE_Pawns, FVector(0,0,0) ) )
	{
		...
		for( i=0; i<MAX_CORONA_LIGHTS; i++ )
		{
			...
			if( Light && CoronaLights[i].iActor<=Viewport->Actor->XLevel->Num() && Viewport->Actor->XLevel->Actors(CoronaLights[i].iActor)==Light )
			{
				...
				if( Loc.Z > 1.0 )
				{
					...
					// high res). Pre-divide by UIScale so DrawTile scales it back to the
