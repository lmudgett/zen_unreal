...
void FDynamicChunk::Filter( UViewport* Viewport, FSceneNode* Frame, INT iNode, INT Outside )
{
	...
	{	
		...
		if( Y1 != Y0 )
		{
			...
			// x64 port: a near-horizontal split line at >2048-px widths can push the
			// 16.16 slope past INT range (float->int overflow = 0x80000000 garbage X
			// clip on the crossing rows). Saturate; 4096 px/scanline already exceeds
			// any useful clip at 8K width.
			FloatFixDX = Clamp( FloatFixDX, -65536.f*4096.f, +65536.f*4096.f );
...
void URender::DrawActorSprite( FSceneNode* Frame, FDynamicSprite* Sprite )
{
	...
	&&	(Sprite->Actor->Texture) )
	{
		...
		// x64 port: ScreenX/Y and the scales are PHYSICAL pixels from projection,
		// but DrawIcon's DrawTile multiplies by the fixed-UI scale (Frame->Y/720
		// above 720p) -- pre-divide so world sprites (torch flames, explosion and
		// impact flashes, light halos) land where projected instead of drifting
