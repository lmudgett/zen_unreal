...
//				Samples[i].Project( Frame );
		...
		// x64 port: DrawIcon runs through the canvas' fixed-UI scaling (DrawTile
		// multiplies by UIScale = Frame->Y/720 above 720p), but these particle
		// positions/sizes are already PHYSICAL pixels from Project() -- pre-divide
		// so they land where projected. Without this, above 720p every bParticles
		// vertex-sprite (blood bursts, bubbles, sparks, teleport FX) was drawn at
		// UIScale times its screen position -- usually entirely off-screen, so
		// enemies appeared to shed no blood at high resolutions. (Same class of
		// bug as the corona fix in UnRender.cpp; no-op at/below 720p.)
		...
		for( i=0; i<Count; i++ )
		{
			if( !SortedPts[i]->Flags )
			{
				...
				(
					Owner->Texture,
					(SortedPts[i]->ScreenX - XSize/2) / UIScale,
					(SortedPts[i]->ScreenY - XSize/2) / UIScale,
					XSize / UIScale,
					YSize / UIScale,
					SpanBuffer,
					Samples[i].Point.Z,
