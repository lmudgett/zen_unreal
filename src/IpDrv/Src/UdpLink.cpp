...
void AUdpLink::execBindPort( FFrame& Stack, BYTE*& Result )
{
	...
	if( GInitialized )
	{
		if( GetSocket()==NULL )
		{
			SetSocket( socket( AF_INET, SOCK_DGRAM, IPPROTO_UDP ) );
			if( GetSocket() != INVALID_SOCKET )
			{
				...
				if( setsockopt( GetSocket(), SOL_SOCKET, SO_BROADCAST, (char*)&TrueBuffer, sizeof(TrueBuffer) )==0 )
				{
					...
					if( bind( GetSocket(), (sockaddr*)&Addr, sizeof(Addr) )==0 )
					{
						DWORD NoBlock = 1;
						if( ioctlsocket( GetSocket(), FIONBIO, &NoBlock )==0 )
						{
							// Success.
							*(DWORD*)Result = 1;
							return;
...
void AUdpLink::execSendText( FFrame& Stack, BYTE*& Result )
{
	...
	if( GetSocket() )
	{
		...
		if( sendto( GetSocket(), Str, appStrlen(Str), 0, (sockaddr*)&Addr, sizeof(Addr) )==0 )
		{
