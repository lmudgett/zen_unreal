...
function Killed(pawn killer, pawn Other, name damageType)
{
	...
	// x64 port: log frags so headless (-server) botmatches can be evaluated
	// from the log.
	if ( Other != None )
	{
		if ( (killer == None) || (killer == Other) )
			log("KILL: "$Other.PlayerName$" suicided ("$damageType$") score="$Other.Score);
		else
			log("KILL: "$killer.PlayerName$" killed "$Other.PlayerName$" ("$damageType$") score="$killer.Score);
	}
...
function EndGame()
{
	...
	while( aPawn != None )
	{
		// x64 port: final score dump for headless botmatch evaluation.
		if ( aPawn.bIsPlayer )
			log("SCORE: "$aPawn.PlayerName$" frags="$aPawn.Score$" deaths="$aPawn.DieCount);
		if ( aPawn.IsA('Bots') )
