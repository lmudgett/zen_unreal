...
function Individualize(bots NewBot, int n, int NumBots)
{
	...
	// Set bot's skin.
	// x64 port: only look up a skin when one is explicitly configured.
	// Re-resolving the class default skin's bare name against the external
	// <Mesh>Skins package always failed (those textures live inside UnrealI,
	// or the default is None) and spammed a "Failed to load" warning for
	// every bot spawned.
