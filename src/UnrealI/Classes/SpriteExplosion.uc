...
var int i;
var float FrameTime; // x64 port: flipbook pacing accumulator
...
{
	// x64 port: advance the flipbook at the 35fps reference rate, not per frame.
	FrameTime += 35.0 * DeltaTime;
	if ( FrameTime < 1.0 )
		return;
	FrameTime -= 1.0;
	i++;
