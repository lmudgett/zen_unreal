...
void UEditorEngine::Init()
{
	...
	{
		// x64 port: "-remake=Pkg" forces the named package to be recompiled from
		// its .uc sources even though its .u loads. The loaded .u supplies the
		// meshes/textures/sounds (whose source art is not in the repo), so this
		// is a code-only make: script classes recompile, retail content objects
		// are kept and resaved.
		char Remake[NAME_SIZE]="";
		Parse( appCmdLine(), "REMAKE=", Remake, ARRAY_COUNT(Remake) );
		...
		for( int i=0; i<32; i++ )
		{
			...
			if( GetConfigString( "Editor.EditorEngine", Key, Pkg, ARRAY_COUNT(Pkg) ) )
			{
				...
				UBOOL Loaded = GObj.LoadPackage( NULL, Filename, LOAD_NoWarn|LOAD_KeepImports )!=NULL; // x64 port: see Remake above
				if( !Loaded || appStricmp(Remake,Pkg)==0 )
				{
					// Create package.
					...
					char Spec[256];
					appSprintf( Spec, "..\\src\\%s\\Classes\\*.uc", Pkg ); // x64 port: sources live under src/
					...
					{
						// Import class.
						appSprintf( Filename, "..\\src\\%s\\Classes\\%s", Pkg, *Files(i) ); // x64 port: sources live under src/; deref FString for varargs
						char Temp[256];
					...
					if( ParseParam( appCmdLine(), "H" ) )
					{
						...
						// Export the header.
						appSprintf( Filename, "..\\src\\%s\\Inc\\%sClasses.h", Pkg, Pkg ); // x64 port: sources live under src/
	...
	{
		// Load classes for editing.
		// x64 port: the interactive editor constructs its engine object from
		// the intrinsic C++ EditorEngine class (loading the retail Editor.u
		// class object would copy 1998 32-bit serialized defaults over the
		// x64 layout), so the EditPackages[] config property is never
		// populated by the property system — read the list straight from the
		// ini here, exactly like the -make branch above does.
		{
			INT NumEdit=0;
			...
			if( NumEdit==0 )
			{
				for( INT i=0; i<ARRAY_COUNT(EditPackages); i++ )
				{
					char Key[256];
					appSprintf( Key, "EditPackages[%i]", i );
					GetConfigString( "Editor.EditorEngine", Key, EditPackages[i], ARRAY_COUNT(EditPackages[i]) );
				}
			}
		}
		GObj.BeginLoad();
		...
		// x64 port: this engine object was constructed from the intrinsic
		// class (see above), so its config-driven script properties — the
		// grid/rotgrid Constraints and the C_* color table every editor
		// viewport draws with — were never loaded from [Editor.EditorEngine].
		// The script-mirror offsets drift from the x64 C++ layout (int-sized
		// vtbl stand-ins, 32-bit TArray mirror), so the generic property
		// LoadConfig cannot be trusted for this class; read the values
		// straight into the C++ members instead.
		{
			const char* Sec = "Editor.EditorEngine";
			char Val[256];
			UBOOL bTemp;
			if( GetConfigBool( Sec, "GridEnabled",    bTemp ) ) Constraints.GridEnabled    = bTemp;
			if( GetConfigBool( Sec, "SnapVertices",   bTemp ) ) Constraints.SnapVertices   = bTemp;
			if( GetConfigBool( Sec, "RotGridEnabled", bTemp ) ) Constraints.RotGridEnabled = bTemp;
			GetConfigFloat( Sec, "SnapDistance", Constraints.SnapDistance );
			if( GetConfigString( Sec, "GridSize", Val, ARRAY_COUNT(Val) ) )
				GetFVECTOR( Val, Constraints.GridSize );
			if( GetConfigString( Sec, "RotGridSize", Val, ARRAY_COUNT(Val) ) )
				GetFROTATOR( Val, Constraints.RotGridSize, 1 );
			GetConfigFloat( Sec, "FovAngleDegrees", FovAngle );

			static const char* ColorNames[] =
			{
				"C_WorldBox","C_GroundPlane","C_GroundHighlight","C_BrushWire","C_Pivot",
				"C_Select","C_Current","C_AddWire","C_SubtractWire","C_GreyWire",
				"C_BrushVertex","C_BrushSnap","C_Invalid","C_ActorWire","C_ActorHiWire",
				"C_Black","C_White","C_Mask","C_SemiSolidWire","C_NonSolidWire",
				"C_WireBackground","C_WireGridAxis","C_ActorArrow","C_ScaleBox",
				"C_ScaleBoxHi","C_ZoneWire","C_Mover","C_OrthoBackground"
			};
			FColor* ColorPtrs[] =
			{
				&C_WorldBox,&C_GroundPlane,&C_GroundHighlight,&C_BrushWire,&C_Pivot,
			...
			INT Loaded=0;
			for( INT i=0; i<ARRAY_COUNT(ColorNames); i++ )
				if( GetConfigString( Sec, ColorNames[i], Val, ARRAY_COUNT(Val) ) )
				{
					Parse( Val, "R=", ColorPtrs[i]->R );
					Parse( Val, "G=", ColorPtrs[i]->G );
					Parse( Val, "B=", ColorPtrs[i]->B );
					Parse( Val, "A=", ColorPtrs[i]->A );
					Loaded++;
				}
			debugf( NAME_Init, "Editor config loaded (grid %gx%gx%g, %i colors)",
				Constraints.GridSize.X, Constraints.GridSize.Y, Constraints.GridSize.Z, Loaded );

			// The browser/background icon textures live in Editor.u (imported
			// by EditorEngine.uc's #exec directives); the intrinsic object's
			// pointers were never filled from the class defaults, and the
			// texture browser dereferences Bkgnd/BkgndHi. Resolve by name.
			UPackage* EdPkg = FindObject<UPackage>( NULL, "Editor" );
			if( EdPkg )
			{
				MenuUp  = FindObject<UTexture>( EdPkg, "B_MenuUp" );
				MenuDn  = FindObject<UTexture>( EdPkg, "B_MenuDn" );
				CollOn  = FindObject<UTexture>( EdPkg, "B_CollOn" );
				CollOff = FindObject<UTexture>( EdPkg, "B_CollOf" );
				PlyrOn  = FindObject<UTexture>( EdPkg, "B_PlyrOn" );
				PlyrOff = FindObject<UTexture>( EdPkg, "B_PlyrOf" );
				LiteOn  = FindObject<UTexture>( EdPkg, "B_LiteOn" );
				LiteOff = FindObject<UTexture>( EdPkg, "B_LiteOf" );
				Bad     = FindObject<UTexture>( EdPkg, "Bad" );
				Bkgnd   = FindObject<UTexture>( EdPkg, "Bkgnd" );
				BkgndHi = FindObject<UTexture>( EdPkg, "BkgndHi" );
			}
			debugf( NAME_Init, "Editor icons %s", Bkgnd ? "resolved" : "MISSING" );
		}
