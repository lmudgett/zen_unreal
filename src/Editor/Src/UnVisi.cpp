...
void FEditorVisibility::MakePortals( INT iNode )
{
	...
	MakePortalsClip( iNode, Poly, 0, &FEditorVisibility::AddPortal ); // x64 port: explicit member-pointer syntax
	...
	while( iNode != INDEX_NONE )
	{
		...
		if( (Surf.PolyFlags & PF_Portal) && GEditor->bspNodeToFPoly( Model, iNode, &Poly ) )
		{
			...
			(
				...
				Poly,
				&FEditorVisibility::BlockPortal, // x64 port: explicit member-pointer syntax
				INDEX_NONE
			);
		}
		iNode = Node.iPlane;
	...
	while( iNode != INDEX_NONE )
	{
		...
		if( !(Model->Nodes->Element(iNode).NodeFlags & NF_IsNew) && GEditor->bspNodeToFPoly( Model, iNode, &Poly ) )
		{
			...
			(
				...
				Poly,
				&FEditorVisibility::TagZonePortalFragment, // x64 port: explicit member-pointer syntax
				INDEX_NONE
			);
