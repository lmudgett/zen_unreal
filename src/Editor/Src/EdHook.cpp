...
UBOOL UEditorEngine::HookExec( const char* Cmd, FOutputDevice* Out )
{
	...
	else if( ParseCommand(&Cmd,"ACTORPROPERTIES") )
	{
		if( !ActorProperties )
		{
			...
			ActorProperties->OpenWindow( (HWND)(UINT_PTR)hWndMain );
	...
	else if( ParseCommand(&Cmd,"PREFERENCES") )
	{
		if( !Preferences )
		{
			...
			Preferences->OpenWindow( (HWND)(UINT_PTR)hWndMain );
	...
	else if( ParseCommand(&Cmd,"LEVELPROPERTIES") )
	{
		if( !LevelProperties )
		{
			...
			LevelProperties->OpenWindow( (HWND)(UINT_PTR)hWndMain );
	...
	else if( ParseCommand(&Cmd,"TEXTUREPROPERTIES") )
	{
		...
		if( ParseObject<UTexture>( Cmd, "TEXTURE=", Texture, ANY_PACKAGE ) )
		{
			...
			TextureProperties->OpenWindow( (HWND)(UINT_PTR)hWndMain );
	...
	else if( ParseCommand(&Cmd,"CLASSPROPERTIES") )
	{
		...
		if( ParseObject<UClass>( Cmd, "Class=", Class, ANY_PACKAGE ) )
		{
			...
			ClassProperties->OpenWindow( (HWND)(UINT_PTR)hWndMain );
...
LRESULT APIENTRY VbWindowProc( HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam ) // x64 port: LRESULT
{
	guard(VbWindowProc);
...
void UEditorEngine::EdCallback( DWORD Code, UBOOL Send )
{
	...
	if( hWndCallback )
	{
		if( Send ) SendMessage( (HWND)(UINT_PTR)hWndCallback, WM_CHAR, 32+Code, 0 );
		else       PostMessage( (HWND)(UINT_PTR)hWndCallback, WM_CHAR, 32+Code, 0 );
	}
	unguard;
}
...
extern "C"
{
	DLL_EXPORT void __stdcall EdInitServer( HWND hInWndMain, HWND hInWndCallback )
	{
		...
		hWndCallback = (DWORD)(UINT_PTR)hInWndCallback; // x64 port: HWNDs are 32-bit significant
		hWndMain     = (DWORD)(UINT_PTR)hInWndMain; // x64 port: ditto
