...
public:
	...
	:	Type(InType), ArrayDim(1), PropertyFlags(0), Enum(NULL), MetaClass(NULL) // x64 port: init the union via its pointer member — BitMask(0) left the high half garbage, poisoning UByteProperty::Enum
	{}
...
public:
	...
	FArchive& operator<<( class UObject*& Res )
		{return Serialize(&Res,sizeof(Res));} // x64 port: VM reads pointer-width object refs (XFERPTR); was a 4-byte DWORD emit
