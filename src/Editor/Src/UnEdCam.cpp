	...
	switch( Mode )
	{
		...
		case EM_BrushSnap:
			...
			if( ScaleIsWithinBounds(&Vector,0.05,400.0) )
			{
				...
				if( Viewport->Actor->XLevel->Brush()->Brush->Polys->Num()==0 ) // x64 port: Num is a method
					break;
...
static void DrawButtons( FSceneNode* Frame )
{
	...
	// Menu toggle button.
	PUSH_HIT(Frame,HMenuToggleButton{}); // x64 port: sizeof(T()) is a function type under modern C++
	ButtonX += DrawButton( Frame, (Frame->Viewport->Actor->ShowFlags&SHOW_Menu) ? GEditor->MenuUp : GEditor->MenuDn, ButtonX, 2 );
	POP_HIT(Frame);

	// Player control button.
	PUSH_HIT(Frame,HPlayerControlButton{}); // x64 port: ditto
