...
UBOOL UEditorEngine::SafeExec( const char* InStr, FOutputDevice* Out )
{
	...
	else if( ParseCommand(&Str,"OBJ") )
	{
		if( ParseCommand( &Str, "LOAD" ) )
		{
			...
			if( Parse( Str, "FILE=", TempFname, 80 ) )
			{
				...
				{
					// x64 port: code-only make (-remake) - if the target
					// package already exists in memory (its objects came in
					// with the retail package) and the file is absent, skip
					// the load silently instead of warning per directive.
					if( appFSize(TempFname)<0
							&& GObj.FindObject( UPackage::StaticClass, ANY_PACKAGE, PackageName ) )
						return 1;
	...
	else if( ParseCommand( &Str, "MESHMAP") )
	{
		...
		else if( ParseCommand( &Str, "SETTEXTURE" ) )
		{
			...
			{
				// x64 port: retail scripts use TEXTURE=Default to mean "leave
				// the slot at the engine default"; no texture by that name
				// exists (this warned in 1998 too, harmlessly). Only warn
				// about lookups that name a real texture.
				char TexName[64]="";
				Parse( Str, "TEXTURE=", TexName, ARRAY_COUNT(TexName) );
				if( appStricmp( TexName, "Default" ) != 0 )
					Out->Logf( NAME_ExecWarning, "Missing meshmap, texture, or num (%s)", Str );
			}
			return 1;
		}
	}
...
UBOOL UEditorEngine::Exec( const char* Stream, FOutputDevice* Out )
{
	...
	else if( ParseCommand(&Str,"BRUSH") )
	{
		if( ParseCommand(&Str,"SET") )
		{
			...
			// x64 port: untextured polys (e.g. from the ImGui brush builders)
			// render black — the modern renderer draws nothing for a NULL
			// surface texture. Default them like classic UnrealEd: current
			// texture if one is picked, else the level default.
			{
				UTexture* Fallback = CurrentTexture;
				if( !Fallback && Level->GetLevelInfo() )
					Fallback = Level->GetLevelInfo()->DefaultTexture;
				if( Fallback )
					for( INT i=0; i<Brush->Polys->Num(); i++ )
						if( !Brush->Polys->Element(i).Texture )
							Brush->Polys->Element(i).Texture = Fallback;
			debugf( NAME_Log, "Brush Set: %i polys, default texture %s", Brush->Polys->Num(), Fallback ? Fallback->GetName() : "NONE" );
			}
	...
	if( ParseCommand(&Str,"LEVEL") )
	{
		...
		else if( ParseCommand(&Str,"VALIDATE") )
		{
			...
			if( Level->Model->Nodes->Num() == 0 ) // x64 port: Num is a method
			{
