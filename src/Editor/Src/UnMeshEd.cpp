...
)
{
	...
	// x64 port: code-only make (-remake) - the source model files for retail
	// content are not in the repo. If the mesh already exists (pre-loaded
	// from the retail package), keep it silently instead of logging errors.
	if( GObj.FindObject( UMesh::StaticClass, InParent, MeshName )
			&& appFSize(AnivFname)<0 )
		return;
