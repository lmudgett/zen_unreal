...
//
// x64 port: enumerate the primary display's real resolutions via the modern
// Win32 API and store them (deduplicated by WxH, ascending) in the client's
// mode arrays. This replaces the legacy DirectDraw EnumDisplayModes path, which
// is unreliable on current Windows and capped the list to a handful of low
// resolutions — so high resolutions (1080p, 1440p, 4K, ...) now appear in both
// the native size menu and the in-game "Select Resolution" list. Only modes at
// the desktop color depth and at least 640x480 are offered.
//
static void FindDisplayModes( UWindowsClient* Client )
{
	guard(FindDisplayModes);
	Client->ddNumModes = 0;

	DEVMODE Desktop;
	appMemset( &Desktop, 0, sizeof(Desktop) );
	Desktop.dmSize = sizeof(Desktop);
	EnumDisplaySettings( NULL, ENUM_CURRENT_SETTINGS, &Desktop );
	DWORD DesktopBpp = Desktop.dmBitsPerPel ? Desktop.dmBitsPerPel : 32;

	DEVMODE Mode;
	appMemset( &Mode, 0, sizeof(Mode) );
	Mode.dmSize = sizeof(Mode);
	for( INT i=0; EnumDisplaySettings( NULL, i, &Mode ) && Client->ddNumModes<Client->DD_MAX_MODES; i++ )
	{
		if( Mode.dmBitsPerPel!=DesktopBpp || Mode.dmPelsWidth<640 || Mode.dmPelsHeight<480 )
			continue;
		// Skip duplicates (same WxH at different refresh rates).
		INT j;
		for( j=0; j<Client->ddNumModes; j++ )
			if( (DWORD)Client->ddModeWidth[j]==Mode.dmPelsWidth && (DWORD)Client->ddModeHeight[j]==Mode.dmPelsHeight )
				break;
		if( j==Client->ddNumModes )
		{
			Client->ddModeWidth [Client->ddNumModes] = Mode.dmPelsWidth;
			Client->ddModeHeight[Client->ddNumModes] = Mode.dmPelsHeight;
			Client->ddNumModes++;
		}
	}
...
void UWindowsViewport::OpenWindow( DWORD InParentWindow, UBOOL Temporary, INT NewX, INT NewY, INT OpenX, INT OpenY )
{
	...
	else
	{
		...
		if( !hWnd )
		{
			...
			ParentWindow	= (HWND)(UINT_PTR)InParentWindow;
			Status			= WIN_ViewportOpening;
...
void UWindowsViewport::FindAvailableModes()
{
	...
	// x64 port: enumerate the display's real modes (see FindDisplayModes) rather
	// than the legacy DirectDraw list.
	FindDisplayModes( Client );
	...
	{
		// Windowed resolution items. x64 port: extended past 800x600 with the
		// common modern windowed sizes.
		...
		// Fullscreen resolution items, from the real display mode list.
...
void UWindowsViewport::Repaint()
{
	...
	{
		// x64 port: -framestats — separate the render+present time from the
		// rest of the engine tick so frame spikes can be attributed.
		...
		if( FrameStats )
		{
			...
			if( T1-T0 > 0.020 )
				debugf( "FrameStats: Draw took %.1fms", (T1-T0)*1000.0 );
			static DOUBLE DrawSum = 0.0; static INT DrawCount = 0;
			DrawSum += T1-T0; DrawCount++;
			if( DrawCount == 300 )
			{
				debugf( "FrameStats: draw avg=%.2fms over %i frames", DrawSum*1000.0/DrawCount, DrawCount );
				DrawSum = 0.0; DrawCount = 0;
			}
		}
		else
			Client->Engine->Draw( this, 0 );
...
UBOOL UWindowsClient::Exec( const char* Cmd, FOutputDevice* Out )
{
	...
	{
		// x64 port: return the real display resolution list (enumerated via
		// EnumDisplaySettings, including high resolutions) that the in-game
		// "Select Resolution" menu populates from. Previously the non-DirectDraw
		// path returned a hardcoded list capped at 800x600.
		if( ddNumModes==0 )
			FindDisplayModes( this );
		FString Result;
...
UBOOL UWindowsViewport::Exec( const char* Cmd, FOutputDevice* Out )
{
	...
	else if( ParseCommand(&Cmd,"SetRes") )
	{
		...
		if( X && Y )
		{
			// x64 port: apply as a fullscreen mode when already fullscreen (the
			// OpenGL path sets FullscreenViewport but not FullscreenhWndDD);
	...
	{
		// The advanced-settings dialog is a normal Win32 window, so drop out of
		// fullscreen first — otherwise it would be hidden behind the fullscreen
		// game window. x64 port: remember that we were fullscreen (and at what
		// resolution) so we can return to fullscreen once the dialog is closed
		// (restored in UWindowsClient::Tick when ConfigProperties goes NULL).
		if( Client->FullscreenViewport )
		{
			Client->RestoreFullscreenAfterPrefs = 1;
			Client->RestoreFullscreenX          = Client->ViewportX;
			Client->RestoreFullscreenY          = Client->ViewportY;
			Client->RestoreFullscreenViewport   = this;
			Client->EndFullscreen();
		}
		if( !Client->ConfigProperties )
		{
			// x64 port: the app is per-monitor DPI-aware (for crisp fullscreen),
			// but this legacy property dialog lays its controls out in raw
			// pixels with no DPI scaling of its own, so under that context it
			// renders tiny on a high-DPI display. Create it under a DPI-unaware
			// thread context so Windows bitmap-scales the whole dialog to the
			// monitor's scale (correct size), then restore the thread context.
			typedef HANDLE (WINAPI *FSetThreadDpiCtx)( HANDLE );
			HMODULE User32 = GetModuleHandleA( "user32.dll" );
			FSetThreadDpiCtx SetThreadDpiCtx = User32 ? (FSetThreadDpiCtx)GetProcAddress( User32, "SetThreadDpiAwarenessContext" ) : NULL;
			HANDLE PrevCtx = NULL;
			if( SetThreadDpiCtx )
				PrevCtx = SetThreadDpiCtx( (HANDLE)(INT_PTR)-1 );	// DPI_AWARENESS_CONTEXT_UNAWARE
			...
			if( SetThreadDpiCtx && PrevCtx )
				SetThreadDpiCtx( PrevCtx );
		}
...
//
LRESULT UWindowsViewport::WndProc( UINT iMessage, WPARAM wParam, LPARAM lParam )
{
	...
	switch( iMessage )
	{
		...
		case WM_DESTROY:
		{
			...
			{
				::SetParent( hWnd, NULL );
				SetFocus( (HWND)(UINT_PTR)ParentWindow );
			}
		...
		case WM_COMMAND:
		{
      		...
      		switch( wParam )
			{
				...
				default:
				{
					// x64 port: bound by the actual enumerated mode count (up to
					// DD_MAX_MODES) instead of the old fixed 10-item range.
					if( wParam>=ID_DDMODE0 && wParam<(WPARAM)(ID_DDMODE0+Client->ddNumModes) )
		...
		case WM_SETFOCUS:
		{
			...
			// x64 port: focus returns to the game when the advanced-options
			// dialog closes — restore fullscreen here if it was open from
			// fullscreen.
			Client->RestoreFullscreenIfPending();
...
void UWindowsClient::EndFullscreen()
{
	...
	if( Viewport )
	{
		...
		{
			// x64 port: undo the borderless-fullscreen changes MakeFullscreen
			// applied for a direct-rendering device — restore the desktop
			// display mode and the saved windowed style before repositioning.
			ChangeDisplaySettings( NULL, 0 );
			SetWindowLongPtr( Viewport->hWnd, GWL_STYLE, Viewport->SavedStyle );
			SetWindowPos( Viewport->hWnd, NULL, 0,0,0,0, SWP_FRAMECHANGED|SWP_NOMOVE|SWP_NOSIZE|SWP_NOZORDER );
...
void UWindowsViewport::MakeFullscreen( INT NewX, INT NewY, UBOOL UpdateProfile )
{
	...
	{
		// x64 port: real fullscreen for a direct-rendering device (OpenGL). The
		// original engine did nothing here but grab the mouse, so fullscreen was
		// visually a no-op. Instead: switch the display to the requested mode
		// (falling back to a borderless window at the current desktop size if
		// that mode isn't available) and make the window a borderless popup that
		// covers the screen. The GL context follows the resized window, and
		// WM_SIZE is suppressed while FullscreenViewport is set, so the frame
		// buffer size is applied explicitly below.
		if( Client->FullscreenViewport )
			Client->EndFullscreen();

		SavedStyle = GetWindowLongPtr( hWnd, GWL_STYLE );

		// x64 port: identify the PRIMARY monitor explicitly so fullscreen always
		// lands on it in a multi-monitor setup — regardless of which monitor the
		// window currently sits on. The primary monitor is the one containing
		// the virtual-desktop origin (0,0). We use its device name for the mode
		// change and its rectangle for placement, so this works even when the
		// primary is not the leftmost monitor.
		MONITORINFOEX mi;
		appMemset( &mi, 0, sizeof(mi) );
		mi.cbSize = sizeof(mi);
		POINT ptPrimary = { 0, 0 };
		...
		INT ScreenX, ScreenY;
		DEVMODE dm;
		appMemset( &dm, 0, sizeof(dm) );
		dm.dmSize       = sizeof(dm);
		dm.dmPelsWidth  = NewX;
		dm.dmPelsHeight = NewY;
		dm.dmFields     = DM_PELSWIDTH | DM_PELSHEIGHT;
		if( ChangeDisplaySettingsEx( mi.szDevice, &dm, NULL, CDS_FULLSCREEN, NULL )==DISP_CHANGE_SUCCESSFUL )
		{
			ScreenX = NewX;
			ScreenY = NewY;
			GetMonitorInfo( hMon, &mi );	// refresh rect after the mode change
		}
		else
		{
			// Requested mode unavailable: borderless at the primary's current size.
			ScreenX = mi.rcMonitor.right - mi.rcMonitor.left;
			ScreenY = mi.rcMonitor.bottom - mi.rcMonitor.top;
		}

		Client->FullscreenViewport = this;
		Client->FullscreenhWndDD   = NULL;
		SetWindowLongPtr( hWnd, GWL_STYLE, WS_POPUP | WS_VISIBLE );
		SetWindowPos( hWnd, HWND_TOP, mi.rcMonitor.left, mi.rcMonitor.top, ScreenX, ScreenY, SWP_FRAMECHANGED | SWP_SHOWWINDOW );
		SetFrameBufferSize( ScreenX, ScreenY, ColorBytes, BLIT_DEFAULT );
		SetDrag( 1 );
...
//
//
// x64 port: return to the fullscreen mode that was active when the
// advanced-options dialog was opened, once that dialog has closed. Closing it
// (WM_CLOSE -> DestroyWindow) destroys the window but leaves the C++ object with
// a NULL hWnd — and NotifyDestroy (which nulls ConfigProperties) only fires on
// object deletion — so the close is detected by the window handle going
// invalid. The dead object is then deleted (which also lets a later reopen
// recreate it) before re-entering fullscreen. Called from WM_SETFOCUS (the
// moment focus returns to the game as the dialog closes) and from Tick.
//
void UWindowsClient::RestoreFullscreenIfPending()
{
	guard(UWindowsClient::RestoreFullscreenIfPending);
	if( !RestoreFullscreenAfterPrefs )
		return;
	if( ConfigProperties && !IsWindow(ConfigProperties->hWnd) )
	{
		delete ConfigProperties;	// ~WWindow -> NotifyDestroy nulls ConfigProperties
		ConfigProperties = NULL;
	}
	if( !ConfigProperties && !FullscreenViewport )
	{
		RestoreFullscreenAfterPrefs = 0;
		INT Index;
		UWindowsViewport* Viewport = RestoreFullscreenViewport;
		RestoreFullscreenViewport = NULL;
		if( Viewport && Viewports.FindItem(Viewport,Index) )
			Viewport->MakeFullscreen( RestoreFullscreenX, RestoreFullscreenY, 1 );
	}
	unguard;
}
...
void UWindowsClient::Tick()
{
	...
	// x64 port: also poll here in case the game is ticking without a focus
	// change (the primary trigger is WM_SETFOCUS when the dialog closes).
	RestoreFullscreenIfPending();
