...
//
// x64 port / security: WavData is untrusted (.uax package content). Scan for a
// chunk by FOURCC with every access bounds-checked. Each chunk header must lie
// fully within the buffer and every advance is computed in 64-bit so a crafted
// ChunkLen cannot walk the pointer out of bounds or overflow it. Returns NULL
// if no in-bounds chunk with the id is found.
static FRiffChunk* FindRiffChunk( BYTE* Base, BYTE* End, DWORD FourCC )
{
	BYTE* Ptr = Base + 3*4;	// past 'RIFF', master size, 'WAVE'
	while( Ptr+8 <= End )
	{
		FRiffChunk* Chunk = (FRiffChunk*)Ptr;
		if( Chunk->ChunkID == FourCC )
			return Chunk;
		QWORD Advance = (QWORD)8 + (((QWORD)Chunk->ChunkLen + 1) & ~(QWORD)1);
		if( (QWORD)(Ptr-Base) + Advance > (QWORD)(End-Base) )
			break;
		Ptr += Advance;
	}
	return NULL;
...
UBOOL FWaveModInfo::ReadWaveInfo( TArray<BYTE>& WavData )
{
	...
	// x64 port / security: require a full RIFF/WAVE header before touching it.
	if( WavData.Num() < (INT)sizeof(FRiffWaveHeader) )
		return 0;

	FFormatChunk* FmtChunk;
	FRiffWaveHeader* RiffHdr = (FRiffWaveHeader*)&WavData(0);
	BYTE* WaveDataBase = &WavData(0);
	WaveDataEnd = WaveDataBase + WavData.Num();
	...
	// Look for the 'fmt ' chunk; it must hold a full FFormatChunk.
	FRiffChunk* RiffChunk = FindRiffChunk( WaveDataBase, WaveDataEnd, mmioFOURCC('f','m','t',' ') );
	if( !RiffChunk || (BYTE*)RiffChunk + 8 + sizeof(FFormatChunk) > WaveDataEnd )
		return 0;
	...
	RiffChunk = FindRiffChunk( WaveDataBase, WaveDataEnd, mmioFOURCC('d','a','t','a') );
	if( !RiffChunk )
		return 0;
	...
	// x64 port / security: clamp the declared data size to the bytes actually
	// present. A crafted 'data' ChunkLen (e.g. 0xFFFFFFF0) would otherwise drive
	// a multi-GB over-read in the caller's memcpy.
	if( SampleDataStart > WaveDataEnd )
		return 0;
	if( SampleDataSize > (DWORD)(WaveDataEnd - SampleDataStart) )
		SampleDataSize = (DWORD)(WaveDataEnd - SampleDataStart);
	SampleDataEnd   =  SampleDataStart+SampleDataSize;

	NewDataSize	= SampleDataSize;

	// Look for an optional 'smpl' chunk; only read the loop info if the whole
	// FSampleChunk plus its first FSampleLoop fit in the buffer.
	RiffChunk = FindRiffChunk( WaveDataBase, WaveDataEnd, mmioFOURCC('s','m','p','l') );
	if( RiffChunk && (BYTE*)RiffChunk + 8 + sizeof(FSampleChunk) + sizeof(FSampleLoop) <= WaveDataEnd )
	{
