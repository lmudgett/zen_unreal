...
void APawn::physicsRotation(FLOAT deltaTime, FVector OldVelocity)
{
	...
	{
		// x64 port: dither the truncated fraction (see AActor::physicsRotation)
		// so slow turn rates do not stall at high framerates.
		int deltaYaw = appFloor( RotationRate.Yaw * deltaTime + appFrand() );
...
void AActor::physicsRotation(FLOAT deltaTime)
{
	...
	// Accumulate a desired new rotation.
	FRotator NewRotation = Rotation;	
	// x64 port: FRotator components are integers, so RotationRate*deltaTime
	// truncates every tick - at 200fps rates below ~200 units/sec never
	// accumulate a whole unit (actor stops rotating) and all slow rotators
	// lag by up to fps units/sec. Dither the fraction so the expected rate
	// is exact at any framerate.
	FLOAT RotDither = appFrand();
	FRotator deltaRotation;
	deltaRotation.Pitch = appFloor( RotationRate.Pitch * deltaTime + RotDither );
	deltaRotation.Yaw   = appFloor( RotationRate.Yaw   * deltaTime + RotDither );
	deltaRotation.Roll  = appFloor( RotationRate.Roll  * deltaTime + RotDither );
...
void APawn::physWalking(FLOAT deltaTime, INT Iterations)
{
	...
	// x64 port: track the zone-current drift included in this move so the
	// end-of-move velocity reconstruction can exclude it (see physSwimming -
	// re-absorbing the current each tick made rivers dt-dependently strong).
	FVector ZoneDrift(0,0,0);
	if ( IsA(APlayerPawn::StaticClass) || (Region.Zone->ZoneVelocity.SizeSquared() > 90000) )
	{
		...
		ZoneDrift = Region.Zone->ZoneVelocity;
		ZoneDrift.Z = 0.0;	// DesiredMove.Z is zeroed below
	}
	DesiredMove.Z = 0.0;
	//-------------------------------------------------------------------------------------------
	//Perform the move
		...
		else
		{
					...
					if (Hit.Time == 1.0)  
					{
						...
						if (Hit.Time < 1.0) //found a ledge
						{
							if ( bAvoidLedges )
							{
								...
								bMoveForward = 0;
								// x64 port: per-tick penalty scaled to the
								// 1998 ~35fps reference tick.
								MoveTimer -= 0.25 * Min(1.f, timeTick*35.f);
							}
							else
							{
							...
							if ( bMoveForward ) //check if should just move forward
							{
								...
								else 
								{
									...
									if ( appFrand() < 2 * timeTick )
										...
										// x64 port: per-tick penalty scaled to
										// the 1998 ~35fps reference tick.
										MoveTimer -= 0.1 * Min(1.f, timeTick*35.f);
								}
							}
			...
			if (Hit.Time < 1.0) //try to step up
			{
				...
				if ( Physics == PHYS_Falling ) // pawn decided to jump up
				{
					...
					// x64 port: guard 0/0 -> NaN when the submove is degenerate.
					if ( DesiredDist > 0.f )
		...
		if ( !bZeroMove )
		{
			...
			else
			{
				...
				else // falling
				{
					...
					FLOAT ActualDist = (Location - subLoc).Size2D();
					// x64 port: guard 0/0 -> NaN when the submove is degenerate
					// (reachable at very small frame deltas).
					if ( DesiredDist > 0.f )
						remainingTime += timeTick * (1 - Min(1.f,ActualDist/DesiredDist));
					Velocity.Z = 0.0;
					eventFalling();
					...
					if ( !bMustJump && (Physics == PHYS_Falling) )
					{
						...
						// x64 port: require a meaningful time base - a denormal
						// denominator turned the velocity astronomic and then NaN.
						// Zone-current absorption scaled to the 1998 ~35fps tick
						// (see physSwimming).
						if (!bJustTeleported && (deltaTime - remainingTime > 0.0001f))
							Velocity = (Location - OldLocation)/(deltaTime - remainingTime)
									 - ZoneDrift * (1.f - Min(1.f, deltaTime*35.f));
						Velocity.Z = velZ;
						if (remainingTime > 0.01)
							physFalling(remainingTime, Iterations);
						return;
	...
	// x64 port: zone-current absorption scaled to the 1998 ~35fps tick
	// (see physSwimming).
	if (!bJustTeleported)
...
void AActor::physFalling(FLOAT deltaTime, INT Iterations)
{
	...
	while ( (remainingTime > 0.0) && (Iterations < 8) )
	{
		...
		FVector Adjusted;
		// x64 port: track the zone-current drift included in this move so the
		// velocity reconstructions below can scale its per-tick absorption to
		// the 1998 ~35fps reference tick (see physSwimming).
		FVector ZoneDrift(0,0,0);
		...
		else if ( Hit.Time < 1.0 )
		{
			...
			else
			{
				...
				else
				{
					...
					// x64 port: zone-current absorption scaled to the 1998
					// ~35fps tick (see physSwimming).
					OldVelocity = (Location - OldLocation)/timeTick
								- ZoneDrift * (1.f - Min(1.f, timeTick*35.f));
					OldVelocity.Z = OldZ;
				}
			}
		}
...
void APawn::physFlying(FLOAT deltaTime, INT Iterations)
{
	...
	if (!bJustTeleported)
		Velocity = (Location - OldLocation) / deltaTime
				 - ZoneVel * (1.f - Min(1.f, deltaTime*35.f));
...
void APawn::physSwimming(FLOAT deltaTime, INT Iterations)
{
	...
	// x64 port: require a meaningful time base (see physWalking) - a denormal
	// denominator turns the velocity astronomic then NaN at high frame rates.
	if (!bJustTeleported && (deltaTime - remainingTime > 0.0001f))
	{
		int bWaterJump = (velZ != Velocity.Z); //changed by script
		if (bWaterJump)
			velZ = Velocity.Z;
		// x64 port: the move included ZoneVel (Adjusted above), and the 1998
		// reconstruction re-absorbed the FULL current into Velocity every tick
		// regardless of tick length - so the current's strength scaled with
		// framerate (~6x retail at this port's 200fps cap: unswimmable).
		// Retail's tuned river feel IS this absorption at the original ~35fps
		// tick, so scale the absorbed share to the 1998 reference rate: at
		// dt>=1/35 this is exactly the original code (full absorption); at
		// higher fps the per-SECOND absorption matches original 35fps play.
		Velocity = (Location - OldLocation) / (deltaTime - remainingTime)
				 - ZoneVel * (1.f - Min(1.f, deltaTime*35.f));
		if (bWaterJump)
			Velocity.Z = velZ;
...
void AActor::physRolling(FLOAT deltaTime, INT Iterations)
{
	...
	// x64 port: remember the zone-current drift included in this move so the
	// end-of-move velocity reconstructions can scale its per-tick absorption
	// to the 1998 ~35fps reference tick (see physSwimming).
	FVector ZoneDrift = Region.Zone->ZoneVelocity;
	FVector DesiredMove = Velocity + ZoneDrift;
	OldLocation = Location;
	bJustTeleported = 0;
	...
	while ( (remainingTime > 0.0) && (Iterations < 8) )
	{
		...
		if ((DropTime == 1.0) || (DropHitZ < 0.7)) //then falling
		{
			...
			if (Physics == PHYS_Falling)
			{
				// x64 port: guard denormal time base (NaN velocity at high FPS).
				// Zone-current absorption scaled to the 1998 ~35fps tick.
				if (!bJustTeleported && (deltaTime - remainingTime > 0.0001f))
					Velocity = (Location - OldLocation)/(deltaTime - remainingTime)
							 - ZoneDrift * (1.f - Min(1.f, deltaTime*35.f));
...
void APawn::physSpider(FLOAT deltaTime, INT Iterations)
{
	...
	// x64 port: guard 0/0 -> NaN dir (stationary spider) that would reach the
	// collision-hash assert; SafeNormal does not sanitize NaN, so stop it here.
	FVector DesiredDir = (MoveSize > 0.0001f) ? DesiredMove/MoveSize : FVector(0,0,0);
		...
		if (!Delta.IsNearlyZero())
		{
			...
			if (Hit.Time < 1.0) 
			{
				...
				else if ( (Floor | realDir) > 0.7 )
				{
					...
					{
						FLOAT velZ = Velocity.Z;
						// x64 port: guard denormal time base (NaN velocity, high FPS).
						if (!bJustTeleported && (deltaTime - remainingTime > 0.0001f))
							Velocity = (Location - OldLocation)/(deltaTime - remainingTime);
						Velocity.Z = velZ;
						if (remainingTime > 0.005)
