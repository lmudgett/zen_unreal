...
void APawn::SuggestJumpVelocity(FVector Dest, FVector &vel)
{
	...
	if (ticks > 0.0)
	{
		...
		// x64 port: velsize is 0 when Dest is directly above/below the pawn;
		// the unguarded 0/0 made Location go NaN, which then tripped the
		// NaN!=NaN "moved without proper hashing" check in the collision hash
		// (reliably reproduced by DM bots jump-scanning DmAriza).
		if (velsize > 0.01)
		{
			vel = vel/velsize;
