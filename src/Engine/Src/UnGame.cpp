...
ULevel* UGameEngine::LoadMap( const FURL& URL, UPendingLevel* Pending, char* Error256 )
{
	...
	guard(Cleanup);
	// x64 port: silence any effects channels started since the SetViewport call
	// above (script Destroyed/teardown events can PlaySound during level exit)
	// before the old level's actors and sounds are garbage-collected out from
	// under the audio bookkeeping (GC bypasses DestroyActor, so NoteDestroy is
	// never called for them).
	if( Audio && Client && Client->Viewports.Num() )
		Audio->SetViewport( Client->Viewports(0) );
	Flush();
	GObj.CollectGarbage( GSystem, RF_Intrinsic );
	unguard;
