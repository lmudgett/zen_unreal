...
void UTexture::GetInfo( FTextureInfo& TextureInfo, DOUBLE CurrentTime )
{
	...
	// x64 port / security: FTextureInfo.Mips is a fixed MAX_MIPS array. The mip
	// count is file-controlled, so a texture declaring more than MAX_MIPS mips
	// would overflow the (typically stack-allocated) Mips[] and NumMips. Clamp
	// both to MAX_MIPS.
	INT NumMipsClamped			= Min( GetNumMips(), (INT)MAX_MIPS );
	TextureInfo.NumMips			= NumMipsClamped;
	TextureInfo.Palette			= GetColors();
	for( INT i=0; i<NumMipsClamped; i++ )
	{
...
void UTexture::Tick( FLOAT DeltaSeconds )
{
	...
	if( MaxFrameRate == 0.0 )
	{
		// x64 port: "constant update" meant once per rendered frame, which was
		// ~35fps in 1998 - at this port's 200fps cap fire/water fractal
		// animation ran ~6x fast. Step at the 1998 reference rate instead;
		// identical behavior whenever the frame rate is at or below 35fps.
		Accumulator += DeltaSeconds;
		if( Accumulator >= 1.f/35.f )
		{
			ConstantTimeTick();
			Accumulator = Min( Accumulator - 1.f/35.f, 1.f/35.f );
		}
	}
	else
	{
...
void UPalette::Serialize( FArchive& Ar )
{
	...
	UObject::Serialize( Ar );
	Ar << Colors;
	// x64 port / security: renderers (and GetColors()) assume a full 256-entry
	// palette and read that many. A crafted UPalette with fewer entries would
	// cause an over-read of the Colors buffer. Enforce the documented invariant
	// on load by padding a short palette up to NUM_PAL_COLORS.
	if( Ar.IsLoading() && Colors.Num()<NUM_PAL_COLORS )
		Colors.AddZeroed( NUM_PAL_COLORS - Colors.Num() );
	unguard;
}
...
void UPalette::Flush()
{
	...
	// x64 port: was MakeCacheID(CID_MAX,0,0,~0), passing ~0 as a UObject* to
	// mean "match any frame index"; build the all-ones high mask directly.
	GCache.Flush( MakeCacheID(CID_ColorDepthPalette,0,0,this), (QWORD)CID_MAX | ((QWORD)0xFFFFFFFF << 32) );
