...
void UMesh::Serialize( FArchive& Ar )
{
	...
	// x64 port: retail packages lack sphere radii (see FSphere serializer);
	// rebuild them from the matching bounding boxes.
	if( Ar.IsLoading() && Ar.Ver()<=61 )
	{
		RebuildSphereRadiusFromBox( BoundingSphere, BoundingBox );
		for( INT i=0; i<BoundingSpheres.Num() && i<BoundingBoxes.Num(); i++ )
			RebuildSphereRadiusFromBox( BoundingSpheres(i), BoundingBoxes(i) );
	}
