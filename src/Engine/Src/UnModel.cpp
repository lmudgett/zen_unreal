...
void UDatabase::Serialize( FArchive& Ar )
{
	...
	// x64 port / security: DbNum/DbMax are raw file-controlled INTs. Realloc()
	// sizes the buffer to DbMax while SerializeData writes DbNum records into
	// it, so DbNum>DbMax (or a DbMax that overflows DbMax*ClassRecordSize) is a
	// heap overflow reachable from the BSP model of essentially any map. Reject
	// inconsistent or oversized counts before allocating.
	if( Ar.IsLoading() )
	{
		INT RecordSize = GetClass()->ClassRecordSize;
		if( DbNum<0 || DbMax<DbNum || (QWORD)(DWORD)DbMax*(QWORD)(DWORD)RecordSize > (QWORD)0x7fffffff )
			appErrorf( "UDatabase::Serialize: bad counts %i/%i in %s", DbNum, DbMax, GetFullName() );
		Realloc();
	}
	SerializeData( Ar );
