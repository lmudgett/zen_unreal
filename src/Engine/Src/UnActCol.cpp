...
void FCollisionHash::RemoveActor( AActor* Actor )
{
	...
	{
		// x64 port: diagnostic — print both positions before dying so the
		// divergence pattern (drift vs stomp vs missed re-add) is visible.
		debugf( "RemoveActor: %s Location=(%f %f %f) ColLocation=(%f %f %f)",
			Actor->GetFullName(),
