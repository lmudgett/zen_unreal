...
void ULevel::Tick( ELevelTick TickType, FLOAT DeltaSeconds )
{
	...
	// x64 port: accumulate on the DOUBLE master clock (see UnLevel.h); resync
	// whenever something else (map load/travel/serialization) rewrote
	// TimeSeconds.
	if( TimeSeconds != (FLOAT)PreciseTimeSeconds )
		PreciseTimeSeconds = TimeSeconds;
	PreciseTimeSeconds += DeltaSeconds;
	TimeSeconds = (FLOAT)PreciseTimeSeconds;
