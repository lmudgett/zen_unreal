...
)
{
	...
	// x64 port: scale virtual UI coords (0..ClipX/ClipY) to physical pixels.
	// Texture coords (U/V/UL/VL) are untouched, so a fixed-size glyph/icon is
	// simply blitted larger. UIScale==1 at/below 720p (no-op).
	FLOAT UIScale = Max( 1.f, (FLOAT)Frame->Y / 720.f );
	X *= UIScale; Y *= UIScale; XL *= UIScale; YL *= UIScale;
...
static inline void DrawChar( UCanvas* Canvas, FTextureInfo& Info, INT X, INT Y, INT XL, INT YL, INT U, INT V, INT UL, INT VL, FPlane Color )
{
	...
	// x64 port: glyph positions arrive in VIRTUAL canvas space (Printf accumulates the
	// pen in ClipX/ClipY units), so scale both the pen position and the blit size up to
	// physical pixels here -- exactly as UCanvas::DrawTile does. Without this the text was
	// positioned virtually but blitted at native size, so at 4K the menu/HUD text was tiny
	// and clustered top-left over full-size (already-scaled) background art. Texture coords
	// (U/V/UL/VL source) stay native. UIScale==1 at/below 720p (no-op).
	FLOAT S = Max( 1.f, (FLOAT)Frame->Y / 720.f );
	FLOAT fX=X*S, fY=Y*S, fXL=UL*S, fYL=VL*S, fU=U, fV=V, fUL=UL, fVL=VL;

	// Reject.
	if( fX+fXL<=0.f || fY+fYL<=0.f || fX>=Frame->FX || fY>=Frame->FY )
		return;

	// Clip (adjust source texcoords proportionally to the clipped physical rect).
	if( fX<0.f )
		{FLOAT C=fX*fUL/fXL; fU-=C; fUL+=C; fXL+=fX; fX=0.f;}
	if( fY<0.f )
		{FLOAT C=fY*fVL/fYL; fV-=C; fVL+=C; fYL+=fY; fY=0.f;}
	if( fXL>Frame->FX-fX )
		{fUL+=(Frame->FX-fX-fXL)*fUL/fXL; fXL=Frame->FX-fX;}
	if( fYL>Frame->FY-fY )
		{fVL+=(Frame->FY-fY-fYL)*fVL/fYL; fYL=Frame->FY-fY;}

	// Draw.
	Frame->Viewport->RenDev->DrawTile( Frame, Info, fX, fY, fXL, fYL, fU, fV, fUL, fVL, NULL, Canvas->Z, Color, FPlane(0,0,0,0), PF_NoSmooth | PF_Masked | PF_RenderHint );
}
...
void UCanvas::Update( FSceneNode* InFrame )
{
	...
	// Copy size parameters from viewport. x64 port: expose a VIRTUAL canvas size
	// (raw pixels / UI scale) so the 1998 fixed-pixel menu/HUD/console layout
	// keeps its ~720p proportions but grows at high resolutions instead of
	// rendering as a tiny island on a 4K screen. UIScale==1 at/below 720p, so
	// the common windowed resolution is byte-for-byte unchanged. DrawTile scales
	// virtual coords back to physical pixels (see below).
	Frame = InFrame;
	FLOAT UIScale = Max( 1.f, (FLOAT)Frame->Y / 720.f );
	X = ClipX = Frame->X / UIScale;
	Y = ClipY = Frame->Y / UIScale;
