...
event PlayerInput( float DeltaTime )
{
	...
	// x64 port: mouse deltas are per-frame counts, but UpdateRotation applies
	// aTurn/aLookUp scaled by DeltaTime; normalize to the 35fps reference tick
	// so aim sensitivity no longer varies with framerate.
	MouseScale = MouseScale / FMax(0.1, 35.0 * DeltaTime);
...
event UpdateEyeHeight(float DeltaTime)
{
	...
	{
		smooth = FMax(smooth, FMin(0.35 * 35.0 * DeltaTime, 1.0)); // x64 port: 0.35/tick at 35fps reference; //FIXME - was 0.43, what should it be?
		bJustLanded = false;
	...
	{
		if ( FOVAngle > DesiredFOV )
			FOVAngle = FOVAngle - FMax(245 * DeltaTime, 0.9 * DeltaTime * (FOVAngle - DesiredFOV)); // x64 port: was 7/tick; 7 * 35fps ref tick
		else
			FOVAngle = FOVAngle - FMin(-245 * DeltaTime, 0.9 * DeltaTime * (FOVAngle - DesiredFOV)); // x64 port: was -7/tick; 35fps ref tick
