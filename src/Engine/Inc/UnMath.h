...
public:
	...
	// Serializer.
	// x64 port: was declared with an FPlane& parameter (copy-paste), which
	// duplicated FPlane's own serializer; modern compilers reject that.
	// Retail v60/v61 packages store spheres as a bare center point — the W
	// radius was added to the format after the version-61 bump without a new
	// version number (verified byte-exact against all 53 retail maps and every
	// mesh in UnrealI.u). Loaders needing a real radius rebuild it from their
	// bounding box after serializing.
...
//
// x64 port: retail v60/v61 packages omit the bounding-sphere radius (see the
// FSphere serializer). Rebuild it as the farthest distance from the sphere
// center to the bounding box — shared by UPrimitive/UMesh serialization so the
// formula lives in exactly one place.
//
inline void RebuildSphereRadiusFromBox( FSphere& Sphere, const FBox& Box )
{
	// The farthest point of the box from the (possibly off-center) sphere center
	// is the per-axis farthest corner, which may MIX Min and Max components --
	// taking only the two diagonal corners underestimated the radius for
	// asymmetric bounds (an under-radius sphere over-culls / under-collides).
	FVector Far
	(
		Abs(Sphere.X - Box.Min.X) >= Abs(Sphere.X - Box.Max.X) ? Box.Min.X : Box.Max.X,
		Abs(Sphere.Y - Box.Min.Y) >= Abs(Sphere.Y - Box.Max.Y) ? Box.Min.Y : Box.Max.Y,
		Abs(Sphere.Z - Box.Min.Z) >= Abs(Sphere.Z - Box.Max.Z) ? Box.Min.Z : Box.Max.Z
	);
	Sphere.W = FDist( Sphere, Far );
}
