...
public:
	...
	static LRESULT APIENTRY StaticProc( WNDPROC DefProc, HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam )
	{
	...
	virtual LONG CommandProc( WPARAM wParam, LPARAM lParam )
	{
...
class WINDOW_API WTerminal : public WTerminalBase, public FOutputDevice
{
	...
	void OpenWindow( HWND hWndParent=NULL, UBOOL AppWindow=0 )
	{
		...
		// x64 port: the 1998 hardcoded 512x256 window is stamp-sized on modern
		// high-DPI displays and shows only a few cramped log lines; open at a
		// useful size scaled to the display DPI (capped to the work area).
		INT DpiY = 96;
		...
		INT SizeX = MulDiv( 900, DpiY, 96 );
		INT SizeY = MulDiv( 600, DpiY, 96 );
		RECT WorkArea;
		if( SystemParametersInfo( SPI_GETWORKAREA, 0, &WorkArea, 0 ) )
		{
			SizeX = Min( SizeX, (INT)(WorkArea.right - WorkArea.left) );
			SizeY = Min( SizeY, (INT)(WorkArea.bottom - WorkArea.top) );
		}
		PerformCreateWindowEx
		(
		...
		// x64 port: DEFAULT_GUI_FONT is 96-dpi-sized (unreadable on high-DPI);
		// use a DPI-scaled monospace font for the log text.
		static HFONT hLogFont = NULL;
		if( !hLogFont )
			hLogFont = CreateFont( -MulDiv(10,DpiY,72), 0, 0, 0, FW_NORMAL, 0, 0, 0,
				DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
				CLEARTYPE_QUALITY, FIXED_PITCH|FF_MODERN, "Consolas" );
		if( hLogFont )
			SendMessage( Display, WM_SETFONT, (WPARAM)hLogFont, MAKELPARAM(1,0) );
	...
	void SelectTyping()
	{
		...
		INT Length = Display.GetLength();
		// x64 port: strlen returns 64-bit size_t; keep the math in INT.
		Display.SetSelection( Max(Length-(INT)strlen(Typing),0), Length );
		unguard;
	}
	void UpdateTyping()
	{
...
public:
	...
	static LRESULT APIENTRY StaticDlgReturnZero( HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam )
	{
		return 0;
	}
	static INT_PTR APIENTRY StaticDlgProc( HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam )
	{
		guard(WWindow::StaticWndProc);
...
public:
	...
	QWORD GetId() const
	{
		...
		return (QWORD)(PTRUINT)this + ((QWORD)3<<56); // x64 port: pointer-sized id, tag moved above user-space address bits
		unguard;
	}
	virtual char* GetCaption( char* Result ) const
	{
