...
class DLL_EXPORT UWindowsViewport : public UViewport, public WWindow
{
	...
	INT					SavedCaps;
	INT					SavedX, SavedY;
	LONG_PTR			SavedStyle;		// x64 port: window style saved when going borderless-fullscreen
...
class DLL_EXPORT UWindowsClient : public UClient, public FNotifyHook
{
	...
	// Constants.
	enum {MAX_DD=4};
	// x64 port: raised from 10 so the full list of a modern display's modes
	// (enumerated via EnumDisplaySettings, including high resolutions) fits.
	enum {DD_MAX_MODES=64};
	...
	// x64 port: state for returning to fullscreen after the advanced-options
	// dialog (which forces windowed mode) is closed. Zero-initialized like
	// ConfigProperties.
	UBOOL				RestoreFullscreenAfterPrefs;
	INT					RestoreFullscreenX, RestoreFullscreenY;
	UWindowsViewport*	RestoreFullscreenViewport;

	// x64 port: if the advanced-options dialog (which forces windowed mode) has
	// been closed, return to the fullscreen mode active when it was opened.
	void RestoreFullscreenIfPending();
